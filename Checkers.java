...
public class Checkers
{
                ...
                for(int i=0;i<8;i++){
                    for(int j=0;j<8;j++){
                        if(board[i][j] == 'r' || board[i][j] == 'R'){
                            ...
                            StdDraw.filledCircle(i+.5,j+.5,.4);
                            if(board[i][j] == 'R'){
                                StdDraw.setPenColor(StdDraw.ORANGE);
                                StdDraw.filledCircle(i+.5,j+.5,.2);
                            }
                        } else if (board[i][j] == 'b' || board[i][j] == 'B'){
                            StdDraw.setPenColor(StdDraw.WHITE);
                            StdDraw.circle(i+.5,j+.5,.4);
                            if(board[i][j] == 'B'){
                                          ...
                                          int   endX, int   endY)
        {
            ...
            //must end on empty space
            if(board[endX][endY] != ' '){
                return false;
            }
            //one direction, single space moves and jumps
            //regular piece
            if(board[startX][startY] == 'r'){
                if(endY-startY == 2){
                    if(board[endX-((endX-startX)/2)][endY-((endY-startY)/2)] != 'b'){
                        return false;
                    }
                } else if(endY-startY != 1){
                    return false;
                }
            } else if(board[startX][startY] == 'b'){
                if(endY-startY == -2){
                    if(board[endX-((endX-startX)/2)][endY-((endY-startY)/2)] != 'r'){
                        return false;
                    }
                } else if(endY-startY != -1){
                    return false;
            ...
            if(board[startX][startY] == 'R'){
                if(endY-startY == 2 || endY-startY == -2){
                    if(board[endX-((endX-startX)/2)][endY-((endY-startY)/2)] != 'b' &&
                       board[endX-((endX-startX)/2)][endY-((endY-startY)/2)] != 'B'){
                        return false;
                    }
                ...
                if(endY-startY == -2 || endY-startY == 2){
                    if(board[endX-((endX-startX)/2)][endY-((endY-startY)/2)] != 'r' &&
                       board[endX-((endX-startX)/2)][endY-((endY-startY)/2)] != 'R'){
                        return false;
                    }
                } else if(endY-startY != -1 && endY-startY != 1){
                    return false;
        ...
        public static void main(String[] args)
        {
                ...
                while (true)
                {
                        ...
                        // ONE LAST CHECK BEFORE WE GO BACK FOR THE NEXT MOVE...SEE IF THE GAME IS OVER
